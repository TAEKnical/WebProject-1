...
try {
	...
	if($id != null && $pw != null && $name != null && $email != null){
		if(preg_match("/^[a-zA-Z]+(([ -])?[a-zA-Z])*$/", $name) && preg_match("/^[a-zA-Z0-9]+$/", $id) && preg_match("/^([\w-]+(?:\.[\w-]+)*)@((?:[\w-]+\.)*\w[\w-]{0,66})\.([a-z]{2,6}(?:\.[a-z]{2})?)$/", $email)) {
			...
			if($con->exec("INSERT INTO info (id, pw, name, email) VALUES ($id, $pw, $name, $email);")){
