<?php
	...
	// 관리자 권한이 있으면 답변 완료 버튼을 클릭 시 table에서 checked 칼럼의 값을 1에서 0으로 변경
	// 관리자 권한이 없으면 alert 띄워준 후 qna.php로 되돌아감
	if ($admin['admin'] == '1') {
		$stmt = $connection->prepare("UPDATE question SET checked = 0 WHERE writer = $writer AND created = $created;");
		$stmt->execute();
